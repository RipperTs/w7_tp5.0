...
class User extends UserModel
{
    ...
    {
        // 区分登陆客户端
        $openId = isset(Cache::get($token)['openid']) ? Cache::get($token)['openid'] : Cache::get($token)['open_id'];
     ...
     * @throws \think\exception\DbException
     */
    public function loginH5($userInfo)
    {
        $user_id = $this->registerH5($userInfo);
     ...
     * @throws \think\exception\DbException
     */
    private function registerH5($userInfo)
    {
        $user = self::detail(['open_id' => $userInfo['open_id']]);
        $model = $user ? $user : $this;
        $this->startTrans();
        try {
     ...
     * @throws \think\Exception
     */
    public static function frequencyDec($user_id)
    {
        return self::where('user_id', $user_id)->setDec('frequency');
    }
