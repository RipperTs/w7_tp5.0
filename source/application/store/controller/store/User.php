...
class User extends Controller
{
    ...
    public function add()
    {
        ...
        if ($model->add($this->postData('user'))) {
            return $this->renderSuccess('添加成功', siteUrl('store.user/index'));
        }
    ...
    public function edit($user_id)
    {
        ...
        if ($model->edit($this->postData('user'))) {
            return $this->renderSuccess('更新成功', siteUrl('store.user/index'));
        }
    ...
    {
        if (IS_FOUNDER) {
            return "<script>alert('创始人请到微擎修改用户信息!');window.history.back();</script>";
        }
        // 管理员详情
