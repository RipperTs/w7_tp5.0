...
class Setting extends BaseModel
{
    ...
    public static function getAll($wxapp_id = null)
    {
        ...
        if (!$data = Cache::get('setting_' . $wxapp_id . '_' . MODULE_UNIACID)) {
            $setting = $static::all(compact('wxapp_id'));
            $data = empty($setting) ? [] : array_column(collection($setting)->toArray(), null, 'key');
            Cache::tag('cache')->set('setting_' . $wxapp_id . '_' . MODULE_UNIACID, $data);
        }
        ...
        return [
            ...
            'recharge' => [
                ...
                'values' => [
                    ...
                    'describe' => "1. 账户充值仅限微信在线方式支付，充值次数实时到账；\n" .
                        "2. 账户充值套餐赠送的次数即时到账；\n" .
                        "3. 账户次数有效期：自充值日起至用完即止；\n" .
                        "4. 如会员和次数同时存在，优先使用会员，不会扣减用户次数；\n" .
                        "5. 若还有其他疑问，可拨打客服电话400-000-1234",     // 充值说明
                ],
            ],
            // 积分设置
            SettingEnum::POINTS => [
